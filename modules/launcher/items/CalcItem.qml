...
Item {
    ...
    function onClicked(): void {
        const cachedResult = Qalculator.evaluate(math, false);
        if (cachedResult) {
            Quickshell.execDetached(["wl-copy", cachedResult]);
            root.list.visibilities.launcher = false;
        } else {
            Qalculator.evalAsync(math, false, res => {
                if (res) {
                    Quickshell.execDetached(["wl-copy", res]);
                }
            });
            root.list.visibilities.launcher = false;
        }
    ...
    RowLayout {
        ...
        StyledText {
            ...
            text: root.math.length > 0 ? Qalculator.evaluate(root.math) : qsTr("Type an expression to calculate")

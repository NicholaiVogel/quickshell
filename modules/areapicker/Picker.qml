...
MouseArea {
    ...
    function save(): void {
        const tmpfile = Qt.resolvedUrl(`/tmp/quickshell-picker-${Quickshell.processId}-${Date.now()}.png`);
        CUtils.saveItem(screencopy, tmpfile, Qt.rect(Math.ceil(rsx), Math.ceil(rsy), Math.floor(sw), Math.floor(sh)), path => {
            if (root.loader.clipboardOnly) {
                ...
                Quickshell.execDetached(["notify-send", "-a", "quickshell", "-i", path, "Screenshot taken", "Screenshot copied to clipboard"]);
            } else {
